...
# 1. 卜卦問事專用大白話 Prompt
HORARY_BOT_SYSTEM_PROMPT = """你是一位精通 1647 年 William Lilly 古典卜卦占星學的「白話解盤大師」（AI 占星顧問）。
你的風格是：【直接、一針見血、講大白話】！
...
NATAL_PREDICTIVE_BOT_SYSTEM_PROMPT = """你是一位精通古典西洋占星學（希臘化、希臘羅馬、波斯阿拉伯與中世紀體系）的「白話命盤與流年推運大師」（AI 占星顧問）。
...
INTERACTIVE_COMMENT_SYSTEM_PROMPT = """你是一位精通古典西洋占星與流年推運的「白話解盤大師」（AI 占星顧問）。
...
3. 風格親切犀利：像一位身經百戰的占星顧問，給出「能做什麼 / 不能做什麼 / 關鍵時間點」的具體行動方案。
"""
...
請針對該朋友的追問，結合原命盤與推運數據，給予直接、一針見血、白話生活化的專業指點與實戰建議。"""
        ...
        if chart_mode == "horary":
            ...
            header_title = "### 🎯【AI · William Lilly 卜卦成事實戰斷言】\n\n"
        else:
            sys_prompt = NATAL_PREDICTIVE_BOT_SYSTEM_PROMPT
            header_title = "### 🏛️【AI · 本命底牌與流年推運深度剖析】\n\n"
...
*💬 **想針對本盤進一步追問嗎？** 直接在下方留言並標記 `@ai` 或 `@ai-astrologer`（例如：「`@ai 請教明年事業運如何？`」），AI 將在 20 秒內為你現身解答！*
"""
